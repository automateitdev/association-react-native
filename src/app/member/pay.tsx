...
export default function MemberScreen() {
  ...
  /*
   * Owed now, and assigned-but-not-yet-owed. The server decides which is
   * which - it knows what day it is at the moment the list was built, and a
   * boundary instalment must not land in both because the app asked later.
   */
  const owed = all.filter((due) => due.due);
  const ahead = all.filter((due) => !due.due);
        ...
        {dues.data ? (
          <Section title="Outstanding" first>
            ...
            */}
            ...
            >
              {/*
                WHAT IS OWED TODAY, not everything ever assigned.

                This read `instalment_total` - the sum of every unpaid row -
                and associations assign months ahead. COCSOL's run to 2027-12,
                so a member one instalment late was told "OUTSTANDING
                18,100.00" when they owed 3,100. The rest is a schedule, and
                calling it a debt is a demand for money nobody has asked for.
              */}
              <AmountBreakdown
                instalment={dues.data.meta.due_instalment_total}
                fine={dues.data.meta.due_fine_total}
                total={dues.data.meta.due_grand_total}
                align="left"
              />
            </View>
          </Section>
        ...
        >
          {owed.map((due, index) => (
            <DueRow
              ...
              divider={index < owed.length - 1}
            />
          ))}
        </Section>

        {/*
          MONTHS NOBODY HAS ASKED FOR YET, kept and kept SEPARATE.

          Not hidden: the association assigned them deliberately and paying
          ahead is ordinary - some members clear a year at a time. But mixed
          into the list above they turned one late instalment into eighteen
          rows, and a member scrolling for the thing they owe had to know the
          date to find it.

          Collapsed behind a count, so the screen opens on the answer to "what
          do I owe" and the rest is there for whoever wants it.
        */}
        {ahead.length > 0 ? (
          <Section title="Pay ahead">
            <Disclosure
              title={`${ahead.length} future ${ahead.length === 1 ? 'instalment' : 'instalments'}`}
              meta={`${formatMoney(dues.data?.meta.scheduled_total ?? '0.00')} scheduled, not yet due`}
            >
              {ahead.map((due, index) => (
                <DueRow
                  key={due.fee_assign_id}
